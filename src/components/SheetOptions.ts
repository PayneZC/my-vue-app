...
const hook: Record<string, any> = {
   ...
   * @returns void | boolean
   */
  columnTitleCellRenderBefore: (columnAbc: object, position: {c: number, left: number, width: number, height: number}, ctx: object): (void | boolean) => {
   ...
   * @returns void | boolean
   */
  columnTitleCellRenderAfter: (columnAbc: string,
   ...
   * @returns void | boolean
   */
  cellMousedownBefore: (cell: object,
    position: { r: number, c: number, start_r: number, start_c: number, end_r: number, end_c: number },
    sheet: object,
    ctx: object): (void | boolean) => {
    console.log('cell ===>>> ', cell);
