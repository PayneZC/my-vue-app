 ...
 * @LastEditTime: 2023-10-30 18:57:19
...
<template>
  <div>
    ...
    <div style="display: inline-block; width: 75vw;"><el-input v-model="loginUser" placeholder="随便输入点啥，保证唯一，将来多人协同编辑测试时得用呢" /></div>

    <!-- table 列表 -->
    <h1 class="header">电子表格列表</h1>
    <el-button type="primary" size="small" @click="editShow(undefined)">
      新增
    </el-button>
    <el-table :data="loadData" style="width: 98vw; margin: 10px;" :border="true">
      ...
      <el-table-column fixed="right" label="操作">
        <template #default="scope">
          <el-button link type="primary" size="small" @click="editShow(scope.row)">
            编辑
          </el-button>
          <el-button link type="danger" size="small" @click="delRow(scope.row.id)">
            删除
          </el-button>
          <el-button link type="success" size="small" @click="excelShow(scope.row)">
            excel
          </el-button>
        </template>
      </el-table-column>
...
const handleClose = (done: () => void) => {
  ElMessageBox.confirm('确定要离开当前页面么?')
    .then(() => {
      done()
    })
}
